...
class RCJSoccerRobot:
    ...
    def parse_team_msg(self, packet: str) -> dict:
        ...
        data = {
            "robot_id": unpacked[0],
            "robot_pos": unpacked[1:4],
            "ball_pos": unpacked[3:5]
        }
        return data
    ...
    def send_data_to_team(self, robot_id, robot_pos=[-100, -100], ball_pos=[-100, -100]) -> None:
        ...
        """
        struct_fmt = "idddd"
        data = [robot_id, *list(robot_pos), *list(ball_pos)]
    ...
    def get_gps_coordinates(self) -> Vector2:
        """Get new GPS coordinates

        Returns:
            Vector2 containing x and y values
        """
        gps_values = self.gps.getValues()
        return Vector2(gps_values[0], gps_values[1])
