...
class MyRobot2(RCJSoccerRobot):
    def go_posWithAngle(self, angle):
        ...
        if -90 < angle < 90:
            ...
            if angle > 40:
                ...
                self.right_motor.setVelocity(utils.velocity(-10 - angle/3))
                self.left_motor.setVelocity(utils.velocity(-10 + angle/3))
    def go_positon(self, dest, wait=False):
        ...
        if utils.getDistance(self.robot_pos, self.ball_pos) < 0.2 and wait:
            ...
            self.go_posWithAngle(angle)
    def RaedAlldataes(self):
    ...
    def getTeamData(self):
        ...
        while self.is_new_team_data():
            ...
            if not self.isBall and unpacked[3]:
                ...
                self.isBall = True
        distances = [0, 0, 0]
        distances[0] = utils.getDistance([self.robot_positions[0][0], self.robot_positions[0][1]], self.ball_pos)#B1
        distances[1] = utils.getDistance([self.robot_positions[1][0], self.robot_positions[1][1]], self.ball_pos)#B2
        distances[2] = utils.getDistance([self.robot_positions[2][0], self.robot_positions[2][1]], self.ball_pos)#B3
        ######Find when to be a Mr_goole_keeoer
        if distances[self.robot_id - 1] == max(distances):
            self.Mr_goole_keeper = True
        else:
            self.Mr_goole_keeper = False
        ############################# Define Passor
        #distances[0] > distances[1]=distances[1]//
        #Robot was between two other 
        if(self.name[1] == '1'):
        ...
        if abs(self.last_ball_positon[0] - self.ball_x) < 0.08 and abs(self.last_ball_positon[1] - self.ball_y) < 0.08:
    ...
    def guessNeutralPoint(self):
        ...
        nearest = self.get_lowest_dist(NEUTRAL_SPOTS)
        return NEUTRAL_SPOTS[nearest]
    def defend_for_goole_keeper(self):
        ############################# Goal Keeper kick off position in lack off progress
        if(self.kickoff):
            if(self.ball_y > 0):
                if(abs(-0.1 - self.robot_y) < 0.05 and abs(self.robot_x) < 0.03):
                    self.turn_To_SeePos(self.O_Goal)
                    self.turnTimeout = 0
                else:
                    self.go_positon([0, -0.1])
            else:
                if(abs(-0.4 - self.robot_y) < 0.05 and abs(self.robot_x) < 0.03):
                    self.turn_To_SeePos(self.O_Goal)
                    self.turnTimeout = 0
                else:
                    self.go_positon([0, -0.4])
        ############################# Goal Keeper
        else:
            m = 0
            if self.ball_y > self.last_ball_positon[1]:
                self.goalKeeper_x = self.ball_x
            elif self.ball_y - self.last_ball_positon[1] != 0:
                m = (self.ball_x - self.last_ball_positon[0])/(self.ball_y - self.last_ball_positon[1])
                self.goalKeeper_x = m*(self.T_Goal[1] - self.ball_y) + self.ball_x
            else:
                self.goalKeeper_x = self.ball_x
            if self.goalKeeper_x > 0.5: self.goalKeeper_x = 0.5
            if self.goalKeeper_x <-0.5: self.goalKeeper_x =-0.5
            self.go_positon([self.goalKeeper_x , self.T_Goal[1]], True)
    def attack(self):
        if(self.ball_y > 0):
            ball_circle = utils.Circle({'x': self.ball_x, 'y': self.ball_y},0.09)
            ...
            self.go_positon(go_pos)

        ############################# Dangerus Position
        if self.ball_y < -0.55 and self.robot_y < -0.55:
            if self.robot_y > self.ball_y + 0.08:
                if self.ball_x > 0:
                    self.go_positon([self.ball_x - 0.2, self.ball_y - 0.12])    
                else:
                    self.go_positon([self.ball_x + 0.2, self.ball_y - 0.12])
            elif self.ball_x > 0:
                if self.robot_x < self.ball_x:
                    self.go_positon([self.ball_x, self.ball_y - 0.05])
                else:
                    self.go_positon([self.ball_x, self.ball_y + 0.2])
            else: 
                if(self.robot_x > self.ball_x):
                    self.go_positon([self.ball_x, self.ball_y - 0.05])
                else:
        ...
        ############################# Lack off progress forward robot
            ...
            if abs(self.neutralPos['y'] - 0.12 - self.robot_y) < 0.05 and abs(self.neutralPos['x'] - self.robot_x) < 0.05:
                ...
                self.go_positon([self.neutralPos['x'], self.neutralPos['y'] - 0.12])
        ############################# Go to Pass position
        # elif self.passor and self.corner and not self.extra_only_passor:
        #     if abs((self.ball_y - 0.1) - self.robot_y) < 0.05 and abs(self.O_Goal[0] - self.robot_x) < 0.05:
        #         self.turn_To_SeePos(self.O_Goal)
        #     else:
        #         self.go_positon([self.O_Goal[0], self.ball_y - 0.1])
        ############################# kick the ball in Pass position
        # elif (not self.extra_only_passor) and self.x_robot > 0.2 and self.y_robot > -0.1 and self.y_robot < 0.1 and self.x_ball > self.x_robot - 0.1:
        ...
        ############################# Run away from goal keeper lack off progress position
            ...
            if(self.robot_x > self.ball_x):
                self.go_positon([self.ball_x + 0.5, (self.robot_y + self.ball_y)/2])
            else:
                self.go_positon([self.ball_x - 0.5, (self.robot_y + self.ball_y)/2])
        ############################# go behind the ball
    ...
    def run(self):
        ...
        self.Mr_goole_keeper = False
        self.goalKeeper_x = 0
        self.last_ball_positon = self.ball_pos
        self.role = 'forward'
        ...
        while self.robot.step(TIME_STEP) != -1:
            if self.is_new_data():
                ...
                if self.waitingForKick:
                    ...
                    if self.robot_id == 1:
                        self.go_positon([ 0.3, -0.3], True)
                    elif self.robot_id == 2:
                        self.go_positon([-0.3, -0.3], True)
                    else:
                        self.go_positon([ 0,   -0.65], True)
                elif self.Mr_goole_keeper:
                      self.defend_for_goole_keeper()
                else:
                    self.attack()
                self.last_ball_positon = self.ball_pos