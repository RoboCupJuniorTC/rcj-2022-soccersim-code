...
#---------------------------------------------------------------------------------------------------go_to
    def go_to(self,x_maghsad,y_maghsad):
        ...
        fasele_noghat_b = math.sqrt((robot_y - y_maghsad)**2 +(robot_x - x_maghsad)**2) 
        fasele_noghat_b = fasele_noghat_b * 20    
        ...
        left_error = fasele_noghat_b + darvaze_pos + divar_error
        right_error = fasele_noghat_b - darvaze_pos - divar_error
        self.move(10 + left_error, 10 + right_error)

#---------------------------------------------------------------------------------------------------main_ball
    def main_ball(self):

        if jahate_toop > 90 and jahate_toop < 270:
            self.turn()
        else:
            self.ball_follow()
...
#---------------------------------------------------------------------------------------------------turn
    def turn(self):
        if jahate_toop > 180 and jahate_toop < 270:
            self.go_to(toop_be_zamin_x + 0.17, toop_be_zamin_y)
        elif jahate_toop > 90 and jahate_toop < 180:
            self.go_to(toop_be_zamin_x - 0.17, toop_be_zamin_y)
        else:
            self.go_to(toop_be_zamin_x, toop_be_zamin_y + 0.17)
...
#---------------------------------------------------------------------------------------------------darvazeban
    def darvaze_ban(self):

        global flag_ekhtelaf

        if name[0] == "B":
            if self.fasele_noghat(-0.20, 0.50) < 0.05:
                flag_ekhtelaf = True
            if flag_ekhtelaf == True:
                self.go_to(0.20, 0.50)
            if self.fasele_noghat(0.20, 0.50) < 0.05:
                flag_ekhtelaf = False
            if flag_ekhtelaf == False:
                self.go_to(-0.20, 0.50)
        else:
            if self.fasele_noghat(-0.20, -0.50) < 0.05:
                flag_ekhtelaf = True
...
#---------------------------------------------------------------------------------------------------get_darvaze_angles
    def get_darvaze_angles(self):

        global darvaze_angle_alamat

        darvaze_angle = math.atan2(robot_y + 0.8 , robot_x - 0)
        if darvaze_angle < 0:
            darvaze_angle = 2 * math.pi + darvaze_angle
        darvaze_angle = math.degrees(darvaze_angle)
        darvaze_pos = (darvaze_angle - robot_angle + 360 + 90)%360
        darvaze_pos = (darvaze_pos - 180) % 360
        darvaze_angle_alamat = 360 - ((darvaze_pos + 180) % 360) - 180
    ...
    def run(self):
        ...
        while self.robot.step(TIME_STEP) != -1:
            if self.is_new_data():
                ...
                if kick_off_data == False and jarime_time < 180 and stuck_time < 100:                       
                    if gher_time > 0:
                        self.move(-10,-5)
                    elif lost_flag == False:
                        if strength > 75 and ((robot_y > 0 and name[0] == "B") or (robot_y < 0 and name[0] == "Y")):
                            if jahate_toop > 80 and jahate_toop < 280 and name[0] == "B":
                                self.move(0,0)
                            elif (jahate_toop < 100 or jahate_toop > 260) and name[0] == "Y":
                                self.move(0,0)
                            else:                                
                                if strength > DARVAZE_GHER:
                                    if name[0] == "B":
                                        self.go_to(-toop_be_zamin_x, -0.81-toop_be_zamin_y)
                                    else:
                                        self.go_to(+toop_be_zamin_x, 0.81+toop_be_zamin_y)
                                else:
                                    self.go_to(toop_be_zamin_x,toop_be_zamin_y)
                            ...
                            if name[0] == "B":
                                if doostam_mige == False:
                                    self.go_to(toop_be_zamin_x, max(0.50+toop_be_zamin_y/3.5, 0.5))
                                else:
                                    self.go_to(max(min(toop_be_zamin_x,0.35),-0.35), max(0.50+toop_be_zamin_y/3.5, 0.5))
                            else:
                                if doostam_mige == False:
                                    self.go_to(toop_be_zamin_x, min(-0.50+toop_be_zamin_y/3.5, -0.5))
                                else:
                                    self.go_to(max(min(toop_be_zamin_x,0.35),-0.35), min(-0.50+toop_be_zamin_y/3.5, -0.5))
                    else:
                        self.darvaze_ban()                    
                else:
                    self.move(0,0)                
